...
import {themeColours} from '../data/dummy';
import { useStateContext } from '../contexts/ContextProvider';
import {FiSettings} from 'react-icons/fi';

const ThemeSettings = () => {
  const{setColour,setMode,currentMode,currentColour,setThemeSettings, themeSettings}=useStateContext();
  return (
          ...
          right-0'>
            <div className='float-right h-screen
             bg-white dark:[#484B52] w-400'>
                ...
                <button type="button" onClick={()=>setThemeSettings(false)} style={{
                  ...
                  {themeColours.map((item,index)=>{
                    ...
                    content={item.name} position="TopCenter">
                      <div className='relative mt-2 cursor-pointer flex gap-5 items-center'key={item.name}>
                        ...
                        cursor-pointer' style={{backgroundColor:item.colour}} 
                        onClick={()=>setColour(item.colour)}>
                          <BsCheck className={`ml-2 text-2xl text-white ${item.colour===currentColour ? 'block':'hidden'}`}/>
                        </button>
                      </div>
                    </TooltipComponent>
                    );
